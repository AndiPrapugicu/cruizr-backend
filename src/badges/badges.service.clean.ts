...
export class BadgesService {
  ...
  async getUserStats(userId: number) {
    ...
    try {
      ...
      return {
        ...
        unlocked,
        percentage:
          totalBadges > 0 ? Math.round((unlocked / totalBadges) * 100) : 0,
        rareCount,
        legendaryCount: 0,
  ...
  private async createTestUsers() {
    const testUsers = [
      {
        name: 'CarMaster2024',
        email: 'carmaster@test.com',
        password: 'hashed_password',
        city: 'Bucuresti',
      },
      {
        name: 'SpeedDemon',
        email: 'speeddemon@test.com',
        password: 'hashed_password',
        city: 'Cluj',
      },
      {
        name: 'TuningKing',
        email: 'tuningking@test.com',
        password: 'hashed_password',
        city: 'Timisoara',
      },
      {
        name: 'DriftMaster',
        email: 'driftmaster@test.com',
        password: 'hashed_password',
        city: 'Iasi',
      },
      {
        name: 'RaceQueen',
        email: 'racequeen@test.com',
        password: 'hashed_password',
        city: 'Brasov',
      },
    ];
  ...
  private async createTestBadges() {
    const testBadges = [
      ...
      },
      {
        key: 'speed_demon',
        name: 'Speed Demon',
        description: 'Love for fast cars',
        icon: '🏎️',
        color: '#EF4444',
        isRare: true,
        requiredCount: 1,
      },
      {
        key: 'tuning_master',
        name: 'Tuning Master',
        description: 'Master of car modifications',
        icon: '🔧',
        color: '#8B5CF6',
        isRare: true,
        requiredCount: 5,
      ...
      },
      {
        key: 'car_collector',
        name: 'Car Collector',
        description: 'Owns multiple cars',
        icon: '🏆',
        color: '#10B981',
        isRare: false,
        requiredCount: 3,
  ...
  private async assignTestBadges() {
    const users = await this.userBadgeRepository.query(
      'SELECT id, name FROM users LIMIT 5',
    );
    const badges = await this.badgeRepository.query(
      'SELECT id, key, isRare FROM badges',
    );
    ...
    for (const user of users) {
      ...
      for (let i = 0; i < Math.min(numBadges, badges.length); i++) {
        ...
        try {
          ...
          console.log(
            `⚠️ Badge assignment might already exist for user ${user.name}`,
          );
        }
      }
    }
